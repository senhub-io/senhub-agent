...
<ul class="rn">
<li><span class="tag t-security">Security</span> <span class="tag t-area">Windows</span> The Windows binaries are now <strong>Authenticode-signed</strong> (SENSOR FACTORY SAS, Certum) — both the bare <code>senhub-agent.exe</code> in the ZIPs and the binary the MSI installs, in addition to the already-signed MSI installer. No more SmartScreen "unknown publisher" on manual runs. (#622)</li>
