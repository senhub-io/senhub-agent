...
When the agent is configured with the [OTLP push strategy](otlp.md), the dashboard renders a dedicated **OTLP Pipeline** card next to the probe list with four sub-sections:
...
For an operator-level reference of every field exposed by this card, see [OTLP observability](https://github.com/senhub-io/senhub-agent/blob/dev/docs/admin-guide/OTLP-OBSERVABILITY.md) in the admin guide.
...
- **Governance** is a block below the parameters, the same for every type: who owns what the probe observes, how critical it is, where it is, its lifecycle, and free labels. The label `application` names the application chain the instance belongs to. It is stamped on the probe's entities, metrics and logs; see [Governance per probe](configuration.md#governance-per-probe).
